...
2026-08-03 Itssle Link 실행에서 인접한 NATIVE 슬라이스를 비교했습니다. 본체가
두 슬라이스를 직접 구현한 경우(Sol 기준) $30.14 / 753크레딧, Luna max가 비슷한
두 슬라이스를 완주하고 본체가 조율한 경우 $7.34 / 184크레딧으로, API 달러와
Codex 크레딧 모두 약 **76% 절감**됐습니다. Coder 4기 전체(대기 63회)의
대기·체크 비용은 $1.26으로 전체의 약 1-2%였고, 대기 자체는 토큰을 소모하지
않습니다. 1회 실행·비슷하지만 동일하지 않은 슬라이스·변동 가능한 요금 기준의
실측 증거이며 청구 금액은 아닙니다.
...
```text
Coder 상태와 최근 메시지·추론·도구·토큰 활동 확인
-> running이며 활동 있음: diff가 없어도 계속 대기
-> 분 단위 실제 무활동 구간 확인
-> 같은 에이전트에 비중단 redirect 1회
-> 두 번째 실제 무활동 구간 확인
-> CODER_STALLED
-> 자식 쓰기 중지
...
`wait_agent` timeout은 그 대기 중 최종 결과가 오지 않았다는 뜻일 뿐 실패가
아닙니다. Lead는 diff가 없거나 대기가 끝났다는 이유만으로 `interrupt:true`를
사용하면 안 됩니다. 파일 수정 전 분석과 도구 사용도 실제 진행입니다. 호스트가
활동을 보여주지 못하면 활성 Coder를 종료하지 말고 계속 기다리거나 사용자에게
판단을 요청합니다.
